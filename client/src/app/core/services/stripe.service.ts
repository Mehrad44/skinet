...
export class StripeService {
 ...
 async initializeElements(){
  ...
  async createAddressElement(){
    if(!this.addressElement){
      ...
      if(elements){
        ...
        throw new Error('Stripe Elements has not been loaded');
      }
    }
  ...
  async confirmPayment(confirmationToken: ConfirmationToken){
    ...
    const result = await elements.submit();
        if(result.error) throw new Error(result.error.message);
        const clinetSecret = this.cartService.cart()?.clientSecret;

        if(stripe && clinetSecret){
          ...
          throw new Error('Unable to load stipe');
        }
  }
  ...
  async createPaymentElement(){
    if(!this.paymentElement){
      ...
      if(elements){
        ...
        throw new Error('Elements isnace has not been initialized');
      }
    }
  ...
  createOrUpdatePaymentIntent(){
    ...
    if(!cart) throw new Error('Problme with cart');
